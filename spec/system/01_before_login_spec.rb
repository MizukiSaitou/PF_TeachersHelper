...
describe '[STEP1] ユーザログイン前のテスト' do
  ...
  describe 'ユーザ新規登録のテスト' do
    ...
    context '新規登録成功のテスト' do
      before do
        fill_in 'user[employee_number]', with: Faker::Lorem.characters(number: 10)
      ...
      it '正しく新規登録される' do
        expect { click_button '新規登録' }
        # .to  change(User.all, :count).by(1)　　ここの記載の意味
      end
      it '新規登録後のリダイレクト先が、menu画面になっている' do
        click_button '新規登録'
  ...
  describe 'ユーザログイン' do
    ...
    context 'ログイン成功のテスト' do
      ...
      it 'ログイン後のリダイレクト先が、menuになっている' do
        expect(current_path).to eq '/menu'
      end
    end
    ...
    context 'ログイン失敗のテスト' do
      before do
        fill_in 'user[employee_number]', with: ''
  ...
  describe 'ヘッダーのテスト: ログインしている場合' do
    ...
    before do
      visit new_user_session_path
      fill_in 'user[employee_number]', with: user.employee_number
      fill_in 'user[email]', with: user.email
      fill_in 'user[password]', with: user.password
      click_button 'ログイン'
    end
